...
// calculate light reflected from directional light
vec3 CalcDirLight(DirLight light, vec3 normal, vec3 viewDir) {
...
// calculate light reflected from point light
vec3 CalcPointLight(PointLight light, vec3 normal, vec3 fragPos, vec3 viewDir){
...
// calculate light reflected by spot light
vec3 CalcSpotlight(Spotlight light, vec3 normal, vec3 fragPos, vec3 viewDir) {
    vec3 lightDir = normalize(light.position - fragPos);
    ...
    // ambient calculated in and outside spotlight
    vec3 result = light.ambient * vec3(texture(first_texture, aTexCoord));

    // calculate colour inside the spotlight
